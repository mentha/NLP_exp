...
class Scissor:
    ...
    def graphTag(self, s, g):
        ...
        for e in sorted(g.edges()):
            ...
            g[pn][cn]['weight'] = w
        return g
    def detectname(self, s):
        '''
        Detect name entity by re
        :return:
        '''
        re_number = re.compile('[0-9]+')
        re_name = re.compile( '[王李张刘陈杨黄吴赵周徐孙马朱胡林郭何高罗郑梁谢宋唐许邓冯韩曹曾彭肖蔡潘田董袁于余蒋叶杜苏魏程吕丁沈任姚卢钟姜崔谭廖范汪陆金石戴贾韦夏邱方侯邹熊孟秦白江闫薛尹付段雷黎史龙贺陶顾龚郝毛邵万钱严洪赖武傅莫孔汤向常温康施文牛樊葛 邢安齐易乔伍庞颜倪庄聂章鲁岳翟申殷詹占欧耿关兰焦俞左柳甘祝包宁符阮尚舒纪柯梅童毕凌单季成霍苗裴涂谷曲盛冉翁蓝骆路游靳辛欧管柴蒙鲍华喻祁房蒲滕萧屈饶解牟艾尤时阳穆农司古吉卓车简连芦缪项麦褚窦娄戚岑党宫景卜费冷晏覃卫习席柏米宗代桂瞿全苟闵佟 应臧边卞姬邬和师仇候栾隋刁沙商寇荣巫郎桑丛阎甄敖虞仲池巩明佘查麻苑迟邝封官谈鞠匡惠荆乐冀胥郁南班储原栗燕楚鄢劳谌奚皮蔺楼粟冼盘满闻位厉伊仝区郜海阚花权强帅屠豆朴盖练廉禹井巴漆祖丰支卿国狄平计索宣晋相初门云容敬来扈晁都芮普阙戈浦伏薄鹿邸雍 辜阿羊母乌亓裘修邰杭赫况那宿鲜逯印隆茹诸战慕危玉银亢公嵇哈湛宾勾茅戎利扬於呼居干揭但尉斯元束檀衣信阴展昝智幸奉植富衡尧由哀爱昂熬傲奥把百摆拜邦宝保暴北贝贲本闭碧薜别邴伯博补布步部才采彩菜仓苍藏操曽茶产昌苌畅倡唱钞超巢朝潮沉晨呈承赤敕崇瘳 丑除楮揣传春淳啜慈次从催翠寸达笪答丹旦淡刀道德迪底第佃定东冬懂钭堵端敦顿多朵鄂恩尔佴法番藩凡芳飞斐风酆逢凤俸扶苻洑浮福甫府复淦刚皋杲格庚弓恭拱贡供缑菇贯冠光广归贵呙虢果过还寒汉行蒿好昊浩禾合河荷黑恒弘红宏鸿后厚忽湖虎户滑化怀淮槐环桓宦皇 辉回会火伙基稽及汲戢籍记继暨加佳家嘉郏荚甲菅翦蹇见建剑将降娇矫徼缴教接节杰颉介京经靖静酒咎琚菊巨句具俱剧矍军君俊卡开凯勘堪考科可克空库蒯郐旷奎蒉阔喇稂朗勒蕾类梨礼里理力历立丽励郦莲良粱疗寥列吝泠零另令留庐炉禄伦洛雒闾侣律绿买曼茆卯冒么枚 美梦弥糜弭宓秘密妙闽敏名谬磨墨默木沐牧睦纳娜乃铙能尼年念乜钮鸥偶攀泮逄朋蓬澎骈濮溥柒其奇祈耆綦千乾潜羌谯郄钦琴勤青清庆丘秋求渠璩却荛绕仁日融如汝瑞闰润若撒萨赛伞森厦山闪陕善赏上韶卲绍厍莘神甚慎升生绳圣诗拾矢士世侍是释守首寿殳书疏树双水税 顺思松送素眭睢随所锁塔台太泰郯潭塘桃淘腾藤提遆天帖铁通同徒脱陀庹拓完宛旺望威巍伟隗未蔚问瓮沃无毋吾仵兀西希郗息溪袭洗喜霞仙先贤咸线香湘象宵箫潇晓孝校忻星刑兴雄吁须顼绪续轩禤玄学雪寻郇荀牙雅焉延言岩彦艳宴洋仰养样幺侥药要冶野业依仪夷怡宜乙 蚁弋义奕羿益裔翼英盈营永勇犹油友有迂鱼宇羽庾遇誉毓员源远院月悦越允运恽载宰迮增甑粘斩长仉掌钊招兆肇折真阵镇征正政只职郅治中忠衷种皱邾珠竹竺主壮禚资子紫訾自字纵俎佐].{1,3}')
        for i in range(0, len(s)):
            num = re_number.match(s[i:])
            if num is not None:
                g.add_node((i, i + len(num) - 1))
                #g.add_weight()
            try:
                name = re_name.match(s[i:i+4]):
                    if name is not None:
                        g.add_node((i, i + 1))
                        #g.add_weight()  tiny weight
                        g.addnode((i, i + 2))
                        g.addnode((i, i + 3))

            except IndexError:
                name = re_name.match(s[i:]):
                    count = len(s) - i
